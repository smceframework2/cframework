...
ZEPHIR_INIT_CLASS(Smce_Core_Di_Singleton) {
	...
	zend_declare_property_null(smce_core_di_singleton_ce, SL("reflection"), ZEND_ACC_PRIVATE|ZEND_ACC_STATIC TSRMLS_CC);

	zend_declare_property_null(smce_core_di_singleton_ce, SL("reflectionClassArr"), ZEND_ACC_PRIVATE|ZEND_ACC_STATIC TSRMLS_CC);

	zend_declare_property_null(smce_core_di_singleton_ce, SL("istance"), ZEND_ACC_PUBLIC|ZEND_ACC_STATIC TSRMLS_CC);
...
PHP_METHOD(Smce_Core_Di_Singleton, __construct) {
	...
	if (!ZEPHIR_IS_STRING(_0, "")) {
		...
		zephir_throw_exception_debug(_1, "smce/core/di/singleton.zep", 33 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Smce_Core_Di_Singleton, resolveWhen) {
	...
	zephir_update_static_property_array_multi_ce(smce_core_di_singleton_ce, SL("disSingleton"), &_0 TSRMLS_CC, SL("z"), 1, key);
	ZEPHIR_INIT_VAR(_1);
	_0 = zephir_fetch_nproperty_this(this_ptr, SL("cs"), PH_NOISY_CC);
	zephir_get_class(_1, _0, 0 TSRMLS_CC);
	zephir_update_static_property_array_multi_ce(smce_core_di_singleton_ce, SL("reflectionClassArr"), &key TSRMLS_CC, SL("z"), 1, _1);
	ZEPHIR_INIT_VAR(_2);
	_3 = zephir_fetch_nproperty_this(this_ptr, SL("cs"), PH_NOISY_CC);
	zephir_get_class(_2, _3, 0 TSRMLS_CC);
	ZEPHIR_CALL_FUNCTION(NULL, "class_alias", &_4, _2, key);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Smce_Core_Di_Singleton, getSingleton) {
	...
	_0 = zephir_fetch_static_property_ce(smce_core_di_singleton_ce, SL("disSingleton") TSRMLS_CC);
	if (zephir_array_isset(_0, key)) {
		_1 = zephir_fetch_static_property_ce(smce_core_di_singleton_ce, SL("disSingleton") TSRMLS_CC);
		zephir_array_fetch(&_2, _1, key, PH_NOISY | PH_READONLY, "smce/core/di/singleton.zep", 57 TSRMLS_CC);
		RETURN_CTOR(_2);
	}
	RETURN_MM_BOOL(0);
...
PHP_METHOD(Smce_Core_Di_Singleton, getKeys) {
	...
	_0 = zephir_fetch_static_property_ce(smce_core_di_singleton_ce, SL("disSingleton") TSRMLS_CC);
	zephir_is_iterable(_0, &_2, &_1, 0, 0, "smce/core/di/singleton.zep", 78);
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_2, &_1)
		...
		zephir_array_update_long(&arr, i, &key, PH_COPY | PH_SEPARATE, "smce/core/di/singleton.zep", 74);
		i++;
	}
	RETURN_CCTOR(arr);
...
PHP_METHOD(Smce_Core_Di_Singleton, controllerConstructorParamerters) {
	...
	ZEPHIR_CALL_METHOD(NULL, _0, "__construct", NULL, controller);
	zephir_check_call_status();
	zephir_update_static_property_ce(smce_core_di_singleton_ce, SL("reflection"), &_0 TSRMLS_CC);
	_1 = zephir_fetch_static_property_ce(smce_core_di_singleton_ce, SL("reflection") TSRMLS_CC);
	ZEPHIR_CALL_METHOD(&_2, _1, "getconstructor", NULL);
	zephir_check_call_status();
	if (!ZEPHIR_IS_FALSE(_2)) {
		_3 = zephir_fetch_static_property_ce(smce_core_di_singleton_ce, SL("reflection") TSRMLS_CC);
		ZEPHIR_CALL_METHOD(&_4, _3, "getconstructor", NULL);
		zephir_check_call_status();
...
PHP_METHOD(Smce_Core_Di_Singleton, controllerMethodParamerters) {
	...
	zephir_update_static_property_ce(smce_core_di_singleton_ce, SL("reflection"), &_0 TSRMLS_CC);
	_1 = zephir_fetch_static_property_ce(smce_core_di_singleton_ce, SL("reflection") TSRMLS_CC);
	ZEPHIR_CALL_METHOD(&_2, _1, "getmethod", NULL, action);
	zephir_check_call_status();
	if (!ZEPHIR_IS_FALSE(_2)) {
		_3 = zephir_fetch_static_property_ce(smce_core_di_singleton_ce, SL("reflection") TSRMLS_CC);
		ZEPHIR_CALL_METHOD(&_4, _3, "getmethod", NULL, action);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(&params, _4, "getparameters", NULL);
		zephir_check_call_status();
		if (zephir_fast_count_int(params TSRMLS_CC) > 0) {
