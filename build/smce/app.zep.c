...
PHP_METHOD(Smce_App, __construct) {
	...
	zval *_0, *config = NULL, *key = NULL, *value = NULL, *_1 = NULL, *_2, *_3, *_5, **_8;
	...
	ZEPHIR_CALL_CE_STATIC(&config, smce_core_eventmanager_ce, "pull", &_4, 2, _5);
	zephir_check_temp_parameter(_5);
	zephir_check_call_status();
	if (!ZEPHIR_IS_FALSE(config)) {
		zephir_is_iterable(config, &_7, &_6, 0, 0, "smce/app.zep", 26);
		for (
...
PHP_METHOD(Smce_App, getIP) {
	...
	zval *ip = NULL, *tmp, _0 = zval_used_for_init, *_1 = NULL, *_3 = NULL, *_4 = NULL, *_5;

	ZEPHIR_MM_GROW();

	ZEPHIR_SINIT_VAR(_0);
	ZVAL_STRING(&_0, "HTTP_CLIENT_IP", 0);
	ZEPHIR_CALL_FUNCTION(&_1, "getenv", &_2, 3, &_0);
	zephir_check_call_status();
	if (zephir_is_true(_1)) {
		ZEPHIR_SINIT_NVAR(_0);
		ZVAL_STRING(&_0, "HTTP_CLIENT_IP", 0);
		ZEPHIR_CALL_FUNCTION(&ip, "getenv", &_2, 3, &_0);
		zephir_check_call_status();
	} else {
		ZEPHIR_SINIT_NVAR(_0);
		ZVAL_STRING(&_0, "HTTP_X_FORWARDED_FOR", 0);
		ZEPHIR_CALL_FUNCTION(&_3, "getenv", &_2, 3, &_0);
		zephir_check_call_status();
		if (zephir_is_true(_3)) {
			...
			ZEPHIR_SINIT_NVAR(_0);
			ZVAL_STRING(&_0, ",", 0);
			ZEPHIR_CALL_FUNCTION(&_4, "strstr", NULL, 4, ip, &_0);
			zephir_check_call_status();
			if (zephir_is_true(_4)) {
				ZEPHIR_INIT_VAR(tmp);
				zephir_fast_explode_str(tmp, SL(","), ip, LONG_MAX TSRMLS_CC);
				ZEPHIR_INIT_NVAR(ip);
				zephir_array_fetch_long(&_5, tmp, 0, PH_NOISY | PH_READONLY, "smce/app.zep", 62 TSRMLS_CC);
				zephir_fast_trim(ip, _5, NULL , ZEPHIR_TRIM_BOTH TSRMLS_CC);
			}
		} else {
			ZEPHIR_SINIT_NVAR(_0);
...
PHP_METHOD(Smce_App, redirect) {
	...
	if (term == 0) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "header", &_2, 5, _1);
		zephir_check_call_status();
	} else {
		ZEPHIR_SINIT_VAR(_3);
		ZVAL_LONG(&_3, term);
